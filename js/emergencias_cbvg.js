...
/* Carga datos desde window.GEO (embebido, abre por doble clic) o por fetch (servidor) */
...
function obtenerColorPorRiesgo(riesgo = "") {
    ...
    if (v.includes("prehospital") || v.includes("salud")) return "#e24b4a";   // atención prehospitalaria
    if (v.includes("vial"))                                return "#8e44ad";   // accidente vial
    if (v.includes("incendio") || v.includes("forestal"))  return "#e67e22";   // incendios
    if (v.includes("creciente") || v.includes("inund"))    return "#4db7ff";   // agua
    if (v.includes("movimiento") || v.includes("masa"))    return "#8d6e63";   // movimiento en masa
    if (v.includes("estructural") || v.includes("colapso"))return "#f4c430";   // estructural
    if (v.includes("químic") || v.includes("quimic") || v.includes("peligros") || v.includes("derrame")) return "#c0392b"; // matpel
    if (v.includes("animal") || v.includes("abeja") || v.includes("árbol") || v.includes("arbol")) return "#27ae60"; // biológico/natural
    if (v.includes("masivo"))                              return "#2980b9";   // evento masivo
    return "#2f7a57";
}
...
/* 6. CARGAR EMERGENCIAS ATENDIDAS POR EL CBVG (KMZ -> GeoJSON) */
cargarGeo("emergencias_cbvg", "data/emergencias_cbvg.geojson")
    .then(data => {
        puntosData = data;
...
function llenarSelectorRiesgos(data) {
    ...
    select.innerHTML = `<option value="">Todas las emergencias</option>`;
        ...
        onEachFeature: function (feature, layer) {
            ...
            let popupHTML = `
                <div style="min-width:240px;">
                    <b style="color:#c0392b">🚒 Emergencia atendida por el CBVG</b><br>
                    <b>N.º:</b> ${num}<br>
                    <b>Tipo:</b> ${riesgo}<br>
                    <b>Fecha:</b> ${fecha || "—"}<br>
                    <b>Vereda:</b> ${vereda}
            ...
            if (image && image.trim() !== "") {
                popupHTML += `
                    <br><br>
                    <a href="${image}" target="_blank" title="Ver evidencia completa">
                      <img src="${image}" alt="Evidencia de atención CBVG"
                        style="width:100%; max-width:240px; border-radius:8px;">
                    </a>
                    <div style="font-size:11px;color:#666;margin-top:4px">Evidencia de atención · CBVG</div>
                `;
            }

            popupHTML += `</div>`;

            layer.bindPopup(popupHTML, { maxWidth: 270 });
        }
    });
}
...
function generarTabla(data) {
    ...
    if (!data || !data.features || data.features.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="5">No se encontraron emergencias para el filtro seleccionado.</td>
            </tr>
        `;
        return;
    }
    ...
    data.features.forEach((feature, index) => {
        ...
        const evid = (image && image.trim() !== "")
            ? `<a href="${image}" target="_blank" onclick="event.stopPropagation()">Ver foto</a>`
            : "—";
